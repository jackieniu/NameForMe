...
/** 给测试/调试用：当前队列深度 */
export function pendingCheckDomainWaiters(): number {
  return getBucket().waitQueue.length;
}

/** 给测试/调试用：当前生效速率（tokens/s） */
export function currentCheckDomainRatePerSecond(): number {
  return getBucket().currentRate;
}
...
/**
 * 指数退避：第 n 次（从 0 起）等待 `base * 2^n` 毫秒，最多 `maxDelayMs`。
 * 用于识别到限流错误后重试前的等待。
 */
export function backoffDelayMs(attempt: number, baseMs = 1000, maxDelayMs = 4000): number {
